...
class LTS(AppClass):
    """
    Light Transport Segements learning.
    Inverse-rendering by encourage consistencies between L_o and hat{L}_o.
    """
    ...
    def load_dataset(self):
        assert self.phase in [
            "train",
            "test_nv",
        ], "LTS stage only supports train or test_nv phases."
    ...
    def load_train_model(self):
        ...
        if not os.path.exists(ckpt):  # resume first
            if self.train_ckpt is not None:
                ...
                ckpt = ckpt.replace("fine.LTS", "fine.Fine")
        ...
        if ckpt is None:
            ...
            raise NotImplementedError("LTS stage needs the pre-trained fine model.")
            ...
            self.sampler = RayGroupManager(
                ...
                self.train_bs,
                0,
                uncert_batch_st=batch_st,
                uncert_data_idxs=data_idxs,
            )
    ...
    def learn(self):
        ...
        for self.global_step in pbar:
            ...
            pbr_off_loss = F.mse_loss(
                results["lin/pbr/off"], results["lin/pbr/off_hat"]
            )
            loss += self.weight_lts * pbr_off_loss
            pbr_off_loss = pbr_off_loss.detach().item()

            pbr_emo_loss = F.mse_loss(
                results["lin/pbr/emo"], results["lin/pbr/emo_hat"]
            )
            loss += self.weight_lts * pbr_emo_loss
            pbr_emo_loss = pbr_emo_loss.detach().item()
                ...
                torch.save(
                    {
                        ...
                        "trainer": {
                            ...
                            "batch_st": self.sampler.uncert_batch_st,
                            "data_idxs": self.sampler.uncert_data_idxs,
